...
__all__ = ['InceptionV4ViAtAGELU']

default_cfgs = {
    'inception_v4_viata_gelu': {
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, l, g):
        ...
        c = self.phi(F.gelu(l_ + g_)) # batch_sizex1xWxH
        #print('c', c.shape)
        # compute attn map
...
class InceptionV4ViAtAGELU(nn.Module):
    ...
    def forward(self, x):
        ...
        # x_a1 = F.interpolate(x_a, size=(18, 18), mode='bicubic', align_corners=False)
        # x_c1 = F.interpolate(x_c, size=(9, 9), mode='bicubic', align_corners=False)
...
def _create_inception_v4_viata_gelu(variant, pretrained=False, **kwargs):
    new_layers = kwargs.get('new_layers', 1)
    model = build_model_with_cfg(
        InceptionV4ViAtAGELU, variant, pretrained, default_cfg=default_cfgs[variant],
...
@register_model
def inception_v4_viata_gelu(pretrained=False, **kwargs):
    return _create_inception_v4_viata_gelu('inception_v4_viata_gelu', pretrained, **kwargs)
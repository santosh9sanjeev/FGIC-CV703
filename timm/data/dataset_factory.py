...
from .sdata.task2 import AircraftCars
from .sdata.task3 import FOODDataset
...
def create_dataset(name, root, split='validation', search_split=True, is_training=False, batch_size=None, seed=None, **kwargs):
    ...
    if name.startswith('tfds'):
        ...
        if name == 'aircraft':
            ds = Aircraft(root, transform=None, train=is_training, download=False, test=True)
        elif name == 'dtd':
            ...
            ds = Cars(root, transform=None, download=False, train=is_training, test=True, size=10)
        elif name == 'task2':
            ds = AircraftCars(root, train=is_training)
        elif name == 'task3':
            ds = FOODDataset(root, train=is_training)
        elif name == "imagenet":
